import { DEFAULT_MIN_COURSES_PER_SLOT, DEFAULT_MAX_COURSES_PER_SLOT } from './config.js';
...
function initializeConstraints() {
    document.getElementById("minCoursesPerSlot").value = DEFAULT_MIN_COURSES_PER_SLOT;
    document.getElementById("maxCoursesPerSlot").value = DEFAULT_MAX_COURSES_PER_SLOT;
}
...
async function generateTimetable() {
    ...
    try {
        ...
        if (timetable) {
            ...
            statusDiv.innerHTML = '<p class="status_error">Unable to generate conflict-free timetable. Try adjusting constraints or reducing courses.</p>';
        }
...
function populateFilterOptions(timetable) {
    ...
    if (filterType === 'course') {
        const courses = getUniqueCourses(timetable);
...
function resetTimetable() {
    ...
    document.getElementById("filterValue").innerHTML = '<option value="all">All</option>';
